// SubagentStart (matcher: builder): card → in_progress + build_start event.
...
if (!cardId) {
  // fallback: the dispatch protocol queues + assigns before spawning
  const board = (await api('GET', `/board?project=${projectId}`)) ?? [];
  cardId = board.find((c) => c.lane === 'queued')?.id ?? null;
}
if (!cardId) process.exit(0);

const agent = data.agent_type ?? 'builder';
await api('POST', `/task/${cardId}/move`, { lane: 'in_progress', agent });
